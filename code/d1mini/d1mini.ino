...
#define update_ledmatrix_command_with_two_args(command, arg1, arg2) do {Serial.print("{"); Serial.print(command);Serial.print("`");Serial.print(arg1);Serial.print("`");Serial.print(arg2);Serial.println("}"); } while(0)
...
class CaptiveRequestHandler : public AsyncWebHandler {
  public:
    CaptiveRequestHandler() {
      ...
      server.on("/newpanelsize", HTTP_POST, [](AsyncWebServerRequest * request) {
        int params = request->params();
        AsyncWebParameter *c = request->getParam(0);
...
void startWebServer()
{
  ...
  AsyncCallbackJsonWebHandler* settingsSubmitHandler = new AsyncCallbackJsonWebHandler("/settingsSubmit", [](AsyncWebServerRequest * request, JsonVariant & json) {
    ...
    if (request->method() == HTTP_POST) {
      ...
      doc["panelwidth"] = json["panelwidth"];
      doc["panelheight"] = json["panelheight"];
