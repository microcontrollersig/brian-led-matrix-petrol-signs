...
public:
    ...
    int drawFixedChar(int x, int y, char ch);
    int drawVariableChar(int x, int y, char ch);
...
int Bitmap::drawChar(int x, int y, char ch) 
{
  if (fontIsFixed(_font)) 
    return drawFixedChar(x, y, ch);
  
  return drawVariableChar(x, y, ch);
}

int Bitmap::drawVariableChar(int x, int y, char ch)
{
    uint8_t height = fontHeight(_font);
    if (ch == ' ') {
 ...
 */
int Bitmap::drawFixedChar(int x, int y, char ch)
{
    uint8_t height = fontHeight(_font);
