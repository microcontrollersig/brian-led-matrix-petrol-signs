...
class GameDino : public Command
{
  ...
  public:
    ...
    void drawDinoGameObject() {
      int k = 0;
      int l = 0;

      uint8_t *data = (uint8_t *) DinoObjectTypes[objectTypeIndex][objectIndex].obj;
      ...
      for (int i = 0; i < length; i++) {
        ...
        for (int j = 7; j >= 0; j--) {
          ...
          if (pixel != 0) {
            virtualCanvas->setPixel(7 - j + x + k, y + l);
          }
        }
